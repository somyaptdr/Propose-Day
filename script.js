...
button.addEventListener("click", () => {
  if (button.textContent === "Click Me! ❤") {
    ...
    let message = "Yes";
    let whatsappUrl =
      "https://wa.me/+919826899694?text=" + encodeURIComponent(message);
...
function drawText() {
  ...
  if (frameNumber > 2400 && frameNumber < 2700) {
    ...
    if (window.innerWidth < 600) {
      drawTextWithLineBreaks(
        [
          "I Know Forever Seems to be Filmy",
          "Just Hold My Hand And I'll Prove It Mera Baccha",
        ],
        canvas.width / 2,
        canvas.height / 2,
        fontSize,
      ...
      context.fillText(
        "I Know Forever Seems to be Filmy Just Hold My Hand And I'll Prove It Mera Baccha",
        canvas.width / 2,
        canvas.height / 2
      );
    }
  ...
  if (frameNumber >= 2700 && frameNumber < 3000) {
    ...
    if (window.innerWidth < 600) {
      ...
      context.fillText(
        "I Know Forever Seems to be Filmy But Just Hold My Hand And I'll Prove It Mera Baccha",
        canvas.width / 2,
        canvas.height / 2
      );
    }
  ...
  if (frameNumber > 3000 && frameNumber < 99999) {
    ...
    if (window.innerWidth < 600) {
      let text = ["I love you so so so much mera baccha❤️😘 "];
      let x = canvas.width / 2;
      let y = canvas.height / 2;

      for (let i = 0; i < text.length; i++) {
        context.fillText(text[i], x, y + i * lineHeight);
      }
    } else {
      context.fillText(
        "I love you so so so much mera baccha❤️😘 ",
        canvas.width / 2,
        canvas.height / 2
      );
    }
